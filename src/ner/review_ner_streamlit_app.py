...
class DataFilter:
    ...
    def _is_fully_reviewed(self, example: Dict) -> bool:
        ...
        return (
            all(entity.get("reviewed", False) for entity in example.get("entities", []))
    ...
    def filter(
        ...
        if selected_category != "ALL":
            filtered_data = [
                ...
                if any(e["label"] == selected_category for e in ex["entities"])
            ]
        ...
        for ex in filtered_data:
            ...
            if (
                (show_empty and entities)
                or (show_only_unreviewed and not self._has_unreviewed(ex))
                or (show_only_reviewed and not self._is_fully_reviewed(ex))
                or (show_overlaps and not self._has_overlaps(entities))
                or (
                    show_invalid
                    and not self._has_invalid_indices(entities, text_length)
                )
            ):
                continue
...
class DataManager:
    ...
    def __init__(
        self, config: ConfigManager, file_utils: FileUtils, logger: LoggerType
    ):
        self.config = config
    ...
    def save_final_data(self) -> bool:
        ...
        try:
            ...
            st.success(
                f"Data saved to {self.config.ner_path_config.corrected_streamlit_data_path}"
            )
            self.session.set("data_hash", self._compute_data_hash(data))
            self.session.set("save_needed", False)
...
class AppUIController:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            if st.button(
                ...
                on_click=self.data_manager.save_final_data,
            ):
            ...
            if save_auto and self.session.get("save_needed", False):
                current_hash = self.data_manager._compute_data_hash(
                    self.session.get("internal_data", [])
                )
    ...
    def render_filter_controls(
        self,
    ) -> Tuple[Tuple[str, bool, bool, bool, bool, bool, bool, str, str], int]:
        """Renders filter controls for category, pagination, and entity search."""
        st.markdown("**Search by entity ID or Text**")
        entity_search_type = st.radio(
            "Search by:",
            ["text", "id"],
            index=0,
            key="entity_search_type_radio",
            format_func=lambda x: "Text" if x == "text" else "ID",
            help="Select the search type: by entity text or by entity id.",
            horizontal=True,
        )
        ...
        entity_search_query = col_search[0].text_input(
            "Enter text or id fo search",
            value=self.session.get("entity_search_query", ""),
            key="entity_search_query",
            help="Enter a word/phrase or id to filter examples.",
        ...
        selected_category = st.selectbox(
            "Select category",
            self.category_manager.get_all() + ["ALL"],
            index=(
                self.category_manager.get_all().index(
                    st.session_state.get("main_filter_category_selectbox", "ALL")
                )
                if st.session_state.get("main_filter_category_selectbox")
                in self.category_manager.get_all()
                else len(self.category_manager.get_all())
            ),
            key="main_filter_category_selectbox",
            help="Filter examples by entity category.",
        )

        items_per_page = st.slider(
            "Examples per page",
            min_value=1,
            max_value=self.config.ner_config.items_per_page_max,
            value=self.config.ner_config.items_per_page_default,
            key="main_items_per_page_slider",
            help="Adjust the number of examples per page.",
        )

        show_only_unreviewed = st.checkbox(
            "Show only unreviewed",
            key="show_only_unreviewed",
            help="Show examples with unreviewed entities.",
        )
        show_only_reviewed = st.checkbox(
            "Show only reviewed",
            key="show_only_reviewed",
            help="Show examples with all entities reviewed.",
        )
        ...
        with col1:
            show_overlaps = st.checkbox(
                "Overlaps",
                key="filter_overlaps",
                help="Show examples with overlapping entities.",
            )
        with col2:
            show_empty = st.checkbox(
                "Empty", key="filter_empty", help="Show examples with no entities."
            )
        with col3:
            show_invalid = st.checkbox(
                "Invalid indices",
                key="filter_invalid",
                help="Show examples with invalid entity indices.",
            )

        show_unique_reviewed = st.checkbox(
            "Show only unique reviewed",
            key="show_unique_reviewed",
            help="Show only examples with unique reviewed entities (no duplicates based on start, end, and label).",
        )
        ...
        filter_params = (
            ...
            show_only_reviewed,
            show_unique_reviewed,
            self.session.get("committed_entity_search_query", ""),
            self.session.get("entity_search_type", "text"),
        )

        return filter_params, items_per_page

    def render_pagination(
        self, total_items: int, items_per_page: int, position: str = "top"
    ) -> int:
        ...
        with col1:
            st.button(
                "←",
                key=f"prev_page_{position}",
                use_container_width=True,
                disabled=current_page == 1,
                on_click=lambda: goto_page(current_page - 1),
            )
        with col2:
            if current_page > 2:
                st.button(
                    f"{current_page - 2}",
                    key=f"quick_prev2_{position}",
                    use_container_width=True,
                    on_click=lambda: goto_page(current_page - 2),
                )
            else:
                st.write("")
        with col3:
    ...
    def render_statistics(self, data: List[Dict[str, Any]]) -> None:
    ...
    def show_annotated_text(self, annotated_fragments: List, default_fill: str) -> None:
        ...
        has_html_inline = any(
            isinstance(frag, tuple) and frag[1] == "html-inline"
        ...
        if has_html_inline:
            ...
            st.markdown(
                f'<div style="display:inline; word-break:break-word;">{html_line}</div>',
                unsafe_allow_html=True,
            )
        else:
            modified_fragments = [
                (
                    (f"{frag[0]} ✔️", frag[1], frag[2])
                    if isinstance(frag, tuple) and frag[3].get("reviewed", False)
                    else frag
                )
                for frag in annotated_fragments
            ]
    ...
    def render_example(self, example: Dict, original_index: int, page_idx: int) -> None:
        ...
        for ent_idx, (orig_idx, entity) in enumerate(sorted_entities):
            start = self.session.get(
                f"start_{original_index}_{orig_idx}", entity["start"]
            )
            end = self.session.get(f"end_{original_index}_{orig_idx}", entity["end"])
            label = self.session.get(
                f"label_{original_index}_{orig_idx}", entity["label"]
            )
                ...
                entity_spans.append(
                    {
                        ...
                        "label": label,
                        "ent_idx": ent_idx,
                        "reviewed": entity.get("reviewed", False),
                    }
                )
            else:
                st.warning(
                    f"Invalid indices for entity '{entity['label']}' in example {example['id']}"
                )
                ...
                merged.append(
                    {
                        ...
                        "end": span["end"],
                        "labels": [span["label"]],
                        "ent_idxs": [span["ent_idx"]],
                        "reviewed": [span["reviewed"]],
                    }
                )
            else:
                last = merged[-1]
        ...
        for group in merged:
            ...
            if len(group["labels"]) > 1:
                ...
                annotated_fragments.append(
                    (frag, main_label, fill, {"reviewed": reviewed})
                )
            last_idx = group["end"]
        ...
        default_fill = (
            self.config.ner_config.entity_colors.get(entities[0]["label"], "#fff")
            if entities
            else "#fff"
        )
        ...
        if entities:
            ...
            for ent_idx, entity in enumerate(entities):
                with st.container(
                    key=f"entity_editor_container_{original_index}_{ent_idx}"
                ):
                    self._render_entity_editor(
                        ent_idx, entity, original_index, overlaps_for_editor
                    )
    ...
    def _get_overlaps(self, entities: List[Dict]) -> Tuple[Set[int], Set[int]]:
        ...
        duplicates = set()
        seen = {}
        sorted_entities_with_indices = sorted(
            enumerate(entities), key=lambda x: x[1]["start"]
        )
    ...
    def _render_entity_editor(
        self, ent_idx: int, entity: Dict, original_index: int, overlaps: Set[int]
    ) -> None:
        ...
        if entity_text == "Invalid indices":
            st.warning("⚠️ Entity range is out of text bounds!")

        with st.container(border=True):
            if ent_idx in overlaps:
                st.warning("⚠️ This entity overlaps with another. Check ranges!")
            st.markdown(
                f'<span style="text-decoration: underline;">{entity_text.replace(" ", "▮")}</span> {"✔️" if entity.get("reviewed", False) else ""}',
                unsafe_allow_html=True,
            )
            ...
            st.slider(
                ...
                len(full_text),
                (start, end),
                key=f"entity_range_slider_{key_prefix}",
                on_change=lambda: self._handle_range_change(
                    original_index, ent_idx, key_prefix, entity
                ),
            )

            col_label, col_info, col_mark, col_review, col_delete = st.columns(
                [3, 1, 3, 1, 1], vertical_alignment="bottom"
            )
            ...
            with col_label:
                selected_label = st.selectbox(
                    "Label",
                    cat_options + ["+ New category..."],
                    index=(
                        cat_options.index(entity["label"])
                        if entity["label"] in cat_options
                        else 0
                    ),
                    key=f"entity_label_select_{key_prefix}",
                    on_change=lambda: self._handle_label_change(
                        original_index, ent_idx, key_prefix, entity
                    ),
                )
                ...
                if selected_label == "+ New category...":
                    new_cat = st.text_input(
                        "Enter new category", key=f"custom_cat_{key_prefix}"
                    )
            ...
            with col_review:
                st.button(
                    "👁️",
                    key=f"entity_reviewed_btn_{key_prefix}",
                    help="Mark as reviewed",
                    on_click=lambda: self._mark_entity_reviewed(
                        original_index, ent_idx, entity
                    ),
                    disabled=entity.get("reviewed", False),
                    use_container_width=True,
                )
            with col_mark:
                st.button(
                    "Find and mark similar",
                    icon="🔎",
                    key=f"entity_mark_all_btn_{key_prefix}",
                    on_click=lambda: self._mark_all_similar(original_index, entity),
                    use_container_width=True,
                )
            with col_delete:
                if st.button(
                    ...
                    icon="🗑️",
                    help="Delete entity",
                    key=f"entity_delete_btn_{key_prefix}",
                    use_container_width=True,
                ):
    ...
    def _handle_label_change(self, original_index, ent_idx, key_prefix, entity):
        ...
        if entity.get("reviewed", False):
            if st.button("Confirm Change", key=f"confirm_label_{key_prefix}"):
                self._update_entity(
                    original_index, ent_idx, key_prefix, new_label=new_label
                )
    ...
    def _handle_range_change(self, original_index, ent_idx, key_prefix, entity):
        ...
        if entity.get("reviewed", False):
            if st.button("Confirm Change", key=f"confirm_range_{key_prefix}"):
                self._update_entity(
                    original_index, ent_idx, key_prefix, start=start, end=end
                )
                self.session.set("save_needed", True)
                st.rerun()
        else:
    ...
    def _render_new_entity_form(
        self, original_index: int, full_text: str, page_idx: int
    ) -> None:
        ...
        with st.expander("Add new entity by word", icon="➕"):
            ...
            word = st.text_input(
                "Word (manual input)",
                value=self.session.get(word_key, ""),
                key=word_key,
            )
            ...
            selected_words = st.multiselect(
                ...
                default=self.session.get(multiselect_key, []),
                key=multiselect_key,
            )
            cat_options = self.category_manager.get_all()
            label = st.selectbox(
                "Category", cat_options + ["+ New category..."], key=label_key
            )

            if label == "+ New category...":
                new_cat = st.text_input(
                    "Enter new category",
                    key=f"custom_cat_add_{original_index}_{page_idx}",
                )
            ...
            def add_entities(words_to_add: List[str]):
                ...
                for word_to_add in words_to_add:
                    for match in re.finditer(
                        re.escape(word_to_add), full_text, re.IGNORECASE
                    ):
                        start_idx, end_idx = match.span()

                        if not any(
                            e["start"] == start_idx and e["end"] == end_idx
                            for e in entities
                        ):
                            entities.append(
                                {
            ...
            if st.button(
                "Add",
                key=f"btn_add_{original_index}_{page_idx}",
                use_container_width=True,
            ):
                words_to_add = (
                    selected_words if selected_words else ([word] if word else [])
                )
...
class NERDatasetReviewer:
    ...
    def run(self) -> None:
        ...
        st.markdown(
            """<style>.block-container {max-width: 1000px !important; padding-left: 2rem; padding-right: 2rem;}</style>""",
            unsafe_allow_html=True,
        )
        ...
        page = self.ui_controller.render_pagination(
            len(filtered_data), items_per_page, position="top"
        )
        ...
        for idx_in_page, example in enumerate(filtered_data[start:end]):
            ...
            if original_index is not None:
                ...
                st.warning(
                    f"Could not find original index for example ID: {example['id']}"
                )

        self.ui_controller.render_pagination(
            len(filtered_data), items_per_page, position="bottom"
        )
