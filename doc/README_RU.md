...
> CUDA 12.8 необходим для совместимости с pytorch
...
> [!IMPORTANT]
> Настоятельно рекомендуется использовать дообучение модели для перевода, так как это значительно улучшит его качество. Кроме того, дообучение позволит модели лучше понимать контекст и нюансы игры, что обеспечит более точные и естественные переводы. Для дообучения модели необходимо подготовить набор данных с аннотированными примерами и обучить модель на этом наборе данных. Чем больше данных, тем лучше будет работать модель. Тонкая настройка также позволит адаптировать модель к стилю и тону игры, что важно для создания качественной локализации.
...
- Установите `target_lang_code` в `src/config/language.py`
